...
use crate::core::{palette, typography, geometry};
use super::{Scene, DrawOp};
...
pub fn build<R: Rng>(normalized: &str, rng: &mut R, opts: &RenderOptions) -> Result<Scene, LogoGenError> {
    let size = opts.size_px;
    let w = size as f32;
    let h = size as f32;

    let pad = (opts.padding_frac * w).round();
    let inner = geometry::Rect { x: pad, y: pad, w: w - 2.0*pad, h: h - 2.0*pad };
    ...
    let badge_shape = if use_circle {
        geometry::Shape::Circle(geometry::Circle { cx: w/2.0, cy: h/2.0, r: (inner.w.min(inner.h) / 2.0) })
    } else {
        let rx = rng.gen_range(MIN_CORNER_RADIUS..MAX_CORNER_RADIUS) * inner.w;
        geometry::Shape::Rect { rect: inner, rx, ry: rx }
    };
    ...
    let mut ops = Vec::new();
    ops.push(DrawOp::Background { color: palette.background });
    ops.push(DrawOp::ShapeFill { shape: badge_shape, color: palette.primary });
    ...
    Ok(Scene { width: size, height: size, ops })
}
